  ...
  addRoleAction() {
    ...
    if (targetRole === null) {
      ...
      return (
        '"' +
        eventObj.parsed[1] +
        '" is not a known role. Try `!roles` to get a list of all Roles (They are case-sensitive)'
      );
    } else if (disallowedRoles.includes(targetRole.name.toLowerCase())) {
      util.log(
        "User Tried to add a restricted/dissalowed role",
        targetRole.name,
        2
      );
    ...
    channels.map(channel => {
      ...
      if (targetChannel === null) {
        return (
          '"' +
          channel +
          '" is not a known channel. Try `!channels` to get a list of all Channels (They are case-sensitive)'
        );
      }
      ...
      return targetChannel.send(
        sender + " has an announcment: ```" + preparedMessage + "```"
      );
    });
  ...
  verifyAction() {
    ...
    if (validDomains.includes(emailDomain)) {
      ...
      collector.on("collect", m => {
        ...
        const userAlredyOnSystem =
          "This email has already been verified to a discord user.";
        models.Member.findOne({ where: { email } }).then(matchedUserData => {
          if (matchedUserData === null) {
            ...
            const targetRole = eventObj.guild.roles.find(
              "name",
              targetVerifiedRoleName
            );
            eventObj.member.addRole(targetRole).catch(util.log);
      ...
      collector.on("end", collected => {
        const verificationTimeout = `!verify timeout. Clap ${
          collected.author.username
        } in irons!  Let's see how well they dance on the plank!`;
      ...
      const mailOptions = {
        ...
        html: `<table><tr><td><p>Enter the code below into Discord, in the same channel on the Armada Server. Verification will timeout after ${timeoutInMiliseconds /
          1000 /
          60} minutes from first entering the !verify command.</p></td></tr><tr><td><h2>Verification Code: ${code}</h2></td></tr></table>`
      };
      // Call sendMail on sendVerifyCode
      // Pass mailOptions & callback function
      sendVerifyCode.sendMail(mailOptions, (err, info) => {
        const errorMsg =
          "Oops, looks like the email can not be sent. It's not you, it's me. Please reach out to a moderator to help you verify.";
        if (err) {
      ...
      util.log("Code", code, 3);
      return `...What's the passcode? \n\n *eyes you suspicously*\n\n I just sent it to your email, just respond back to this channel within ${timeoutInMiliseconds /
        1000 /
        60} minutes, with the code, and I won't treat you like a scurvy cur!`;
    } else {
