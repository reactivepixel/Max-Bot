...
class RoleController extends BaseController {
  constructor(message) {
    ...
    this.commands = [
      new Command('!roles', '!roles', 'List All Roles', 'List all available Armada roles.', this.rolesAction.bind(controller), 'dm'),
      ...
      new Command('!addRoles', '!addRoles <role_name>,[role_name]', 'Add Multiple Roles', 'Add multiple roles to yourself. Rolea are case-sensitive.', this.addRolesAction.bind(controller)),
      new Command('!removeRole', '!removeRole <role_name>', 'Remove Role', 'Remove a single role from yourself. Role is case-sensitive.', this.removeRoleAction.bind(controller)),
  ...
  addRoleAction() {
    ...
    const targetRole = message
      .guild
      .roles
      .find('name', message.parsed[1]);
    if (targetRole === null) {
      util.log('No role matched', message.parsed[1], 2);
      return '"' + message.parsed[1] + '" is not a known role. Try `!roles` to get a list of all Roles (They are case-se' +
          'nsitive)';
  ...
  addRolesAction() {
    ...
    roles.map((role) => {
      if (!disallowedRoles.includes(role.toLowerCase())) {
        ...
        if (targetRole === null) {
          return '"' + role + '" is not a known role. Try `!roles` to get a list of all Roles (They are case-se' +
              'nsitive)';
        }
        return message
          .member
